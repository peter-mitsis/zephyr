...
void z_impl_k_event_init(struct k_event *event)
{
	...
	k_obj_core_link(K_OBJ_CORE(event));
#endif
}
...
#ifdef CONFIG_OBJ_CORE_EVENT
static int init_event_module(void)
{
	/* Initialize condvar object type */

	z_obj_type_init(&obj_type_event, OBJ_TYPE_EVENT_ID,
...
SYS_INIT(init_event_module, PRE_KERNEL_1,
