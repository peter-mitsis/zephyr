...
int z_impl_k_condvar_init(struct k_condvar *condvar)
{
	...
	k_obj_core_link(K_OBJ_CORE(condvar));
#endif
...
#ifdef CONFIG_OBJ_CORE_CONDVAR
static int init_condvar_module(void)
{
	/* Initialize condvar object type */

	z_obj_type_init(&obj_type_condvar, OBJ_TYPE_CONDVAR_ID,
...
SYS_INIT(init_condvar_module, PRE_KERNEL_1,
